...
let
  ...
  instancesCfg = lib.listToAttrs (
    ...
    ) (lib.mapAttrsToList (k: v: lib.nameValuePair k v) cfg.instances)
  );
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.services = lib.mapAttrs' (
      name: instanceCfg:
      lib.nameValuePair "code-server@${name}" {
        ...
        serviceConfig = {
          ExecStart =
            "${cfg.package}/bin/code-server --socket /run/code-server/${name}.sock --socket-mode 200 "
    ...
    users.users = lib.mapAttrs' (
      ...
      lib.nameValuePair instanceCfg.user (
        lib.mkIf instanceCfg.createUser {
          isNormalUser = true;
          shell = pkgs.zsh;
        }
      )
